...
- **Optional ink snapshots** - Pages containing pen ink can append full-page PDF snapshots generated from OneNote's PDF export
...
5. **Configure options**:
   ...
   - **Append full-page PDF snapshots for pen ink** - Optional; off by default
...
| `--ink-page-snapshots` | Append full-page PDF snapshots for pages containing pen ink (off by default) |
...
# Export pages with pen ink and append full-page PDF snapshots
...
By default, pen ink is not exported, preserving the legacy text-first behavior. Enable the GUI checkbox or pass `--ink-page-snapshots` to detect pages containing OneNote `InkDrawing` content, publish those pages to PDF through OneNote, save the PDF in the assets folder, and append a link to it in the Markdown under a `Page snapshot` heading. Snapshot export is non-blocking: if OneNote cannot publish a snapshot, the normal Markdown export continues and a warning is logged.
